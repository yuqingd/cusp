...
def make_model(floor_size=None, terrain=False, rangefinders=False,
               walls_and_ball=False, maze=False):
  """Returns the model XML string."""
  if maze:
    xml_string = common.read_model('quadruped_maze.xml')
  else:
...
@SUITE.add()
def maze(time_limit=_DEFAULT_TIME_LIMIT, random=None,
           environment_kwargs=None):
  """Returns the Maze task."""
  xml_string = make_model(rangefinders=True, maze=True)
  physics = Physics.from_xml_string(xml_string, common.ASSETS)
  task = Maze(random=random)
  environment_kwargs = environment_kwargs or {}
  return control.Environment(physics, task, time_limit=time_limit,
                             control_timestep=_CONTROL_TIMESTEP,
                             **environment_kwargs)
...
class Maze(base.Task):
  """A quadruped task solved by moving through maze"""
  ...
  def initialize_episode(self, physics):
    ...
    """
    # Initial configuration.
    self._desired_speed = _WALK_SPEED
    azimuth = np.pi/4
    orientation = np.array((np.cos(azimuth/2), 0, 0, np.sin(azimuth/2)))
    _find_non_contacting_height(physics, orientation, x_pos=0.0, y_pos=0.0)
    super().initialize_episode(physics)
